...
# --no-deps matters. PySpin's C extension is built against the numpy 1.x ABI,
# but the wheel declares an unpinned "numpy" -- and --force-reinstall applies
# to dependencies too, so without --no-deps pip cheerfully pulls numpy 2 back
# over the pin and the import fails with "_ARRAY_API not found". Install the
# wheel alone, then choose numpy ourselves, in that order.
"$VENV/bin/pip" install --force-reinstall --no-deps "$whl"

# Pinned here rather than in requirements.txt: numpy 1.26 has no wheel for
# Python 3.13, so an unconditional pin would send setup.sh into a source build
# on a system running the station without a camera. By this point the venv is
# on the wheel's own Python, where 1.26 wheels exist.
say "Pinning numpy to 1.x for PySpin's ABI"
"$VENV/bin/pip" install "numpy<2"

npv="$("$VENV/bin/python" -c 'import numpy;print(numpy.__version__)' 2>/dev/null || echo none)"
echo "   numpy $npv"
case "$npv" in
  1.*) ;;
  *) die "numpy is $npv but PySpin needs 1.x. Something re-installed it:
    $VENV/bin/pip install 'numpy<2'" ;;
esac
